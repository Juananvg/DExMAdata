...
\textbf{IDsDExMA} is a list of 330 dataframes that contains the equivalent official genes symbols to the different IDs or probes. The \emph{Genesymbol}  object of \emph{IDsDExMA} contains other names that have been used as a gene name for an organism.
...
\textbf{SynonymsDExMA} is a data.frame of 2 columns that contains other posible names that a gene can have in a organism. It is similar to the \emph{GeneSymbol} of \emph{IDsDExMA} object.
